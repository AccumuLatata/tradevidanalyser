...
4. **Optional visual notes** — keyframes at chapter markers (`tva frames`,
   ...
   opt-in VLM (`tva vlm`, `TVA_VLM_PROVIDER` unset = off; fake / grok frames
   as `image_url` / gemini File API). Input is redacted frames or clips only.
   Notes may not state a number OCR did not read. Full-frame "watch the
   chart" is out.
...
Decidable from fills alone (deterministic, PR-20):
